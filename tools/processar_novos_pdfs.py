"""
Script para processar NOVOS PDFs (apostilas + Fluxus Continuum atualizado).
...
# Dois locais de PDFs
PDF_DIRS = [
    Path(r"C:\Users\VENDATECH01\Desktop\terapeutas-agent\materiais\APOSTILAS"),
]
PDF_FILES_AVULSOS = [
    Path(r"C:\Users\VENDATECH01\Downloads\O Fluxus Continuum de John Dee PDF.pdf"),
]
...
def criar_chunks(paginas: list[dict], nome_arquivo: str) -> list[dict]:
    """Divide o texto em chunks e mantém metadata (página e arquivo fonte)."""
    chunks = []
    chunk_index = 0
    for pag in paginas:
...
def processar_pdf(caminho: Path) -> dict:
    ...
    if not paginas:
        print("  AVISO: PDF sem texto extraível (escaneado?). Pulando.")
...
def listar_todos_pdfs() -> list[Path]:
    """Lista todos os PDFs dos diretórios e arquivos avulsos."""
    pdfs = []

    # PDFs de diretórios
    for diretorio in PDF_DIRS:
        if diretorio.exists():
            encontrados = sorted([f for f in diretorio.iterdir() if f.suffix.lower() == ".pdf"])
            pdfs.extend(encontrados)
            print(f"  Diretório: {diretorio} -> {len(encontrados)} PDFs")
        else:
            print(f"  AVISO: Diretório não encontrado: {diretorio}")

    # PDFs avulsos
    for arquivo in PDF_FILES_AVULSOS:
        if arquivo.exists():
            pdfs.append(arquivo)
            print(f"  Arquivo avulso: {arquivo.name}")
        else:
...
def main():
    print("=" * 60)
    print("  PROCESSAMENTO DE NOVOS PDFs")
    print(f"  Terapeuta ID: {TERAPEUTA_ID}")
    print("=" * 60)

    # Listar todos os PDFs
    print("\nBuscando PDFs...\n")
    pdfs = listar_todos_pdfs()
    print(f"\nTotal: {len(pdfs)} PDFs para processar:\n")
